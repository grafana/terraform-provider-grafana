...
func Test_NewClient(t *testing.T) {
	...
	t.Run("successfully creates a new client", func(t *testing.T) {
		actualClient := NewClient(&http.Client{}, "test:auth", "https://test.url", defaultHeaders, "test-user-agent")
...
func Test_newHTTPClient(t *testing.T) {
	t.Run("creates a new client with default transport", func(t *testing.T) {
		client := &http.Client{}
		auth := "test:auth"
		headers := map[string]string{"test-key": "test-val"}
		userAgent := "test-user-agent"

		actualClient := newHTTPClient(client, auth, headers, userAgent)
