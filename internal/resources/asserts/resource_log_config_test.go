...
// TestAccAssertsLogConfig_eventualConsistencyStress tests multiple resources created simultaneously
...
func TestAccAssertsLogConfig_eventualConsistencyStress(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAssertsLogConfigStressConfig(stackID, baseName),
				Check: resource.ComposeTestCheckFunc(
					// Check that all resources were created successfully
					testAccAssertsLogConfigCheckExists("grafana_asserts_log_config.test1", stackID, baseName+"-1"),
					testAccAssertsLogConfigCheckExists("grafana_asserts_log_config.test2", stackID, baseName+"-2"),
					testAccAssertsLogConfigCheckExists("grafana_asserts_log_config.test3", stackID, baseName+"-3"),
					resource.TestCheckResourceAttr("grafana_asserts_log_config.test1", "name", baseName+"-1"),
					resource.TestCheckResourceAttr("grafana_asserts_log_config.test2", "name", baseName+"-2"),
					resource.TestCheckResourceAttr("grafana_asserts_log_config.test3", "name", baseName+"-3"),
				),
			},
		},
	})
...
func testAccAssertsLogConfigStressConfig(stackID int64, baseName string) string {
	return fmt.Sprintf(`
resource "grafana_asserts_log_config" "test1" {
  name = "%s-1"
  ...
  config = <<-EOT
    ...
    logConfig:
      tool: "loki"
      url: "https://logs1.example.com"
    defaultConfig: false
  EOT
}

resource "grafana_asserts_log_config" "test2" {
  name = "%s-2"
  ...
  config = <<-EOT
    ...
    logConfig:
      tool: "elasticsearch"
      url: "https://elastic2.example.com"
    defaultConfig: false
  EOT
}

resource "grafana_asserts_log_config" "test3" {
  name = "%s-3"
