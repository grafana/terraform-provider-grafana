...
func TestImportProbeStateWithToken(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			d := schema.TestResourceDataRaw(t, syntheticmonitoring.ResourceProbe().Schema, nil)
			d.SetId(tc.input)

			res, err := syntheticmonitoring.ImportProbeStateWithToken(context.Background(), d, nil)
			switch {
