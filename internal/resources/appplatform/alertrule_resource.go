...
`,
				SpecAttributes: map[string]schema.Attribute{
					...
					"notification_settings": nfSettingsSchema(),
					...
					"panel_ref": schema.SingleNestedAttribute{
						Optional:    true,
...
func nfSettingsSchema() schema.Attribute {
...
func parseAlertRuleSpec(ctx context.Context, src types.Object, dst *v0alpha1.AlertRule) diag.Diagnostics {
	...
	if !data.Expressions.IsNull() && !data.Expressions.IsUnknown() {
		for ref, rawVal := range data.Expressions.Elements() {
			...
			data, diags := parseAlertRuleExpressionModel(ctx, obj)
			if diags.HasError() {
				return diags
			}
...
func saveAlertRuleSpec(ctx context.Context, src *v0alpha1.AlertRule, dst *ResourceModel) diag.Diagnostics {
	...
	if len(src.Spec.Expressions) > 0 {
		exprMap, d := types.MapValueFrom(ctx, ruleExpressionType, src.Spec.Expressions)
		if d.HasError() {
			return d
		}
		values["expressions"] = exprMap
	}
