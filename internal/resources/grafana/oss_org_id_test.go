...
var (
	...
	// so they do not run concurrently and see each other's provider config.
...
// token so callers can pass them in the Terraform provider config (e.g. via testutils.ConfigWithTokenProvider)
