...
func TestAccDataSource_PrometheusDeprecatedSigV4Auth(t *testing.T) {
	testutils.CheckOSSTestsEnabled(t)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					datasourceCheckExists.exists("grafana_data_source.test", &dataSource),
					resource.TestCheckResourceAttr("grafana_data_source.test", "name", dsName),
					resource.TestCheckResourceAttr("grafana_data_source.test", "type", "prometheus"),
				),
...
func TestAccDataSource_PrometheusDeprecatedAzureAuth(t *testing.T) {
	testutils.CheckOSSTestsEnabled(t)
