		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.grafana_cloud_provider_aws_cloudwatch_scrape_job.test", "service.0.enhanced_metric.#", fmt.Sprintf("%d", len(testAWSCloudWatchScrapeJobData.Services[0].EnhancedMetrics))),
					resource.TestCheckResourceAttr("data.grafana_cloud_provider_aws_cloudwatch_scrape_job.test", "service.0.enhanced_metric.0.name", testAWSCloudWatchScrapeJobData.Services[0].EnhancedMetrics[0].Name),
