		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.from_load_test_id", "schedules.0.frequency", "DAILY"),
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.from_load_test_id", "schedules.0.interval", "1"),
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.from_load_test_id", "schedules.0.occurrences", "5"),
					...
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.from_load_test_id", "schedules.1.frequency", "WEEKLY"),
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.from_load_test_id", "schedules.1.interval", "2"),
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.from_load_test_id", "schedules.1.until", "2025-01-31T23:59:59Z"),
...
func TestAccDataSourceK6Schedules_empty(t *testing.T) {
	testutils.CheckCloudInstanceTestsEnabled(t)

	var loadTest k6.LoadTestApiModel

	projectName := "Terraform Empty Schedules Project " + acctest.RandString(8)
	loadTestName := "Terraform Load Test Without Schedules " + acctest.RandString(8)
...
resource "grafana_k6_load_test" "empty_schedules_load_test" {
  project_id = grafana_k6_project.test_project.id
  name       = "%s"
  script     = <<-EOT
    export default function() {
      console.log('Hello from k6 empty schedules test!');
    }
  EOT
}

data "grafana_k6_schedules" "empty_list" {
  load_test_id = grafana_k6_load_test.empty_schedules_load_test.id
}
`, loadTestName),
				Check: resource.ComposeTestCheckFunc(
					loadTestCheckExists.exists("grafana_k6_load_test.empty_schedules_load_test", &loadTest),
					// Data source attributes
					resource.TestCheckResourceAttrSet("data.grafana_k6_schedules.empty_list", "id"),
					resource.TestCheckResourceAttrWith("data.grafana_k6_schedules.empty_list", "load_test_id", checkLoadTestIDMatch),
					resource.TestCheckResourceAttrWith("data.grafana_k6_schedules.empty_list", "id", checkLoadTestIDMatch),
					// Should have no schedules
					resource.TestCheckResourceAttr("data.grafana_k6_schedules.empty_list", "schedules.#", "0"),
				),
			},
		},
	})
