...
func withClientForDataSource(req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) (*assistantapi.Client, error) {
	client, ok := req.ProviderData.(*common.Client)
	if !ok {
		resp.Diagnostics.AddError(
		...
		return nil, fmt.Errorf("unexpected DataSource Configure Type: %T", req.ProviderData)
	}
