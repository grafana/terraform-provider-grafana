...
The unit-tests workflow runs a job **"SDKv2 migration (no new legacy resources)"** that warns when a PR (or push to main) adds new SDKv2 resource or datasource registration in `internal/resources/` (i.e. new `NewLegacySDKResource` or `NewLegacySDKDataSource` calls). New resources/datasources must use the Plugin Framework (`common.NewResource` / Framework datasource pattern). The check is **warning-only** (job does not fail). When the check triggers on a pull request, it also posts a **PR comment** with the migration notice and the added lines that triggered it, so authors see the guidance in the PR conversation. To make the check fail the job once the migration policy is strict, change the step in `.github/workflows/unit-tests.yml` from `exit 0` to `exit 1` in the "Check for new SDKv2 resource/datasource usage" step.
