...
curl -sH "Authorization: Bearer <Access Token from previous step>" "https://grafana.com/api/instances" | \
  jq '[.items[]|{stackName: .slug, clusterName:.clusterSlug, cloudProviderAPIURL: "https://connections-api-\(.clusterSlug).grafana.net"}]'
```
...
For example, in the following response, the correct hostname for the `herokublogpost` stack is `https://connections-api-prod-us-central-0.grafana.net`.
...
Once you have the token and Cloud Provider API hostname, you can configure the provider as follows:

```hcl
provider "grafana" {
  connections_url          = "<Connections API URL from previous step>"
  connections_access_token = "<Access Token from previous step>"
}
```
