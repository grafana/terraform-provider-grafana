---
page_title: "grafana_oncall_on_call_shift Data Source - terraform-provider-grafana"
subcategory: "OnCall"
description: |-
  Data source for Grafana OnCall Shifts. HTTP API https://grafana.com/docs/oncall/latest/oncall-api-reference/on_call_shifts/
---

# grafana_oncall_on_call_shift (Data Source)

* [HTTP API](https://grafana.com/docs/oncall/latest/oncall-api-reference/on_call_shifts/)

## Example Usage

```terraform
data "grafana_oncall_on_call_shift" "example" {
  name = "Primary Rotation"
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `name` (String) The shift's name.

### Read-Only

- `by_day` (Set of String) The list of days in iCal format.
- `by_month` (Set of Number) The list of months.
- `by_monthday` (Set of Number) The list of days of the month.
- `duration` (Number) The duration of the event.
- `frequency` (String) The frequency of the event.
- `id` (String) The ID of this resource.
- `interval` (Number) The positive integer representing at which intervals the recurrence rule repeats.
- `level` (Number) The priority level.
- `rolling_users` (List of Set of String) The list of lists with on-call users (for rolling_users event type).
- `start` (String) The start time of the on-call shift.
- `team_id` (String) The ID of the OnCall team.
- `time_zone` (String) The shift's timezone.
- `type` (String) The shift's type.
- `users` (Set of String) The list of on-call users.
- `week_start` (String) Start day of the week in iCal format.
